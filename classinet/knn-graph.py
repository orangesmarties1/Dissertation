...
if __name__ == "__main__":
    ...
    #   word2: {...}, ... }
    # change this file name to linear/rbf depending on what svm kernel
    # you're using
    graph_dict = {}
    with open(c.output_dir + "edges-directed-rbf.txt") as file:
        for line in file:
            l = line.strip().split()
    ...
    with open(c.output_dir + "edges-directed-rbf-knn.txt", "w") as file:
        for i in knn_edges:
            file.write(i[0] + " " + i[1] + " " + str(i[2]) + "\n")
    
    # Save mutual k-NN edges to new file
    #with open(c.output_dir + "edges-directed-knn-m.txt", "w") as file:
    #    for i in mutual_knn_edges:
    #        file.write(i[0] + " " + i[1] + " " + str(i[2]) + "\n")
